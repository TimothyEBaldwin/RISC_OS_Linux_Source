...
#ifdef DEBUG
    if (mrtdebug)
	Printf("ip_mrouter_init: so_type = %d, pr_protocol = %d\n",
...
X_ip_mrouter_done()
{
	...
	mb_rt = mfctable[i];
	while (mb_rt) {
	    if (mb_rt->m_list != NULL) {
		while (mb_rt->m_list) {
		    m = mb_rt->m_list;
		    mb_rt->m_list = m->m_list;
...
#ifdef DEBUG
	    Printf("source routed tunnels not supported\n");
#endif
	    return EOPNOTSUPP;
	}
    } else {
...
#ifdef DEBUG
    if (mrtdebug)
	Printf("add_vif #%d, lcladdr %x, %s %x, thresh %x, rate %d\n",
	    vifcp->vifc_vifi,
    ...
    struct mfcctl *mfccp;
{
	...
	if (mrtdebug & DEBUG_MFC)
	    Printf("add_mfc update o %x g %x p %x\n",
	...
	if ((rt->mfc_origin.s_addr == mfccp->mfcc_origin.s_addr) &&
...
#ifdef DEBUG
	    if (nstl++)
		Printf("add_mfc %s o %x g %x p %x dbx %x\n",
		    "multiple kernel entries",
		    ntohl(mfccp->mfcc_origin.s_addr),
		    ntohl(mfccp->mfcc_mcastgrp.s_addr),
		    mfccp->mfcc_parent, mb_rt->m_list);

	    if (mrtdebug & DEBUG_MFC)
		Printf("add_mfc o %x g %x p %x dbg %x\n",
		    ntohl(mfccp->mfcc_origin.s_addr),
		    ntohl(mfccp->mfcc_mcastgrp.s_addr),
		    mfccp->mfcc_parent, mb_rt->m_list);
#else
	    ...
	    while (mb_rt->m_list) {
		mb_ntry = mb_rt->m_list;
		rte = mtod(mb_ntry, struct rtdetq *);
/* #ifdef RSVP_ISI */
		ip_mdq(rte->m, rte->ifp, rt, -1);
/* #endif */
		mb_rt->m_list = mb_ntry->m_list;
		m_freem(rte->m);
#ifdef UPCALL_TIMING
		collate(&(rte->t));
#endif /* UPCALL_TIMING */
...
#ifdef DEBUG
	if (mrtdebug & DEBUG_MFC)
	    Printf("add_mfc no upcall h %d o %x g %x p %x\n",
...
#ifdef DEBUG
    if (mrtdebug & DEBUG_MFC)
	Printf("del_mfc orig %x mcastgrp %x\n",
    ...
    struct ip_moptions *imo;
{
	...
	Printf("ip_mforward: src %x, dst %x, ifp %x\n",
...
#ifdef DEBUG
	if ((srctun++ % 1000) == 0)
	    Printf("ip_mforward: received source-routed packet from %x\n",
	...
	if (mrtdebug & (DEBUG_FORWARD | DEBUG_MFC))
	    Printf("ip_mforward: no rte s %x g %x\n",
		ntohl(ip->ip_src.s_addr),
	...
	if (mb_rt == NULL) {
...
#ifdef DEBUG
		Printf("ip_mforward: ip_mrouter socket queue full\n");
#endif
	    ...
	    for (rte_m = mb_rt, npkts = 0; rte_m->m_list; rte_m = rte_m->m_list)
		npkts++;
...
expire_upcalls(void *unused)
{
	...
	for (mb_rt = *nptr; mb_rt != NULL; mb_rt = *nptr) {
		...
		if (mrtdebug & DEBUG_EXPIRE)
		    Printf("expire_upcalls: expiring (%x %x)\n",
...
#ifdef DEBUG
	if (mrtdebug & DEBUG_FORWARD)
	    Printf("wrong if: ifp %x vifi %d vififp %x\n",
    ...
    register struct mbuf *m;
{
    ...
    i = MHLEN(mb_copy) - M_LEADINGSPACE(mb_copy);
    if (i > len)
	i = len;
...
#ifdef DEBUG
	if (mrtdebug)
          Printf("ip_mforward: no tunnel with %x\n",
		ntohl(ip->ip_src.s_addr));
#endif
	return;
    }
    ...
    register struct mbuf *m;
{
	...
	if (mrtdebug & DEBUG_XMIT)
	    Printf("phyint_send on vif %d err %d\n",
		vifp - viftable, error);
#endif
    }
}
...
#else
    socket_send(viftable[vifi].v_rsvpd, m, &rsvp_src);
#endif
