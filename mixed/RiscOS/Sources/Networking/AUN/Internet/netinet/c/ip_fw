...
static int	del_entry(struct ip_fw_head *chainptr, struct ip_fw *frwl);
static struct ip_fw *
...
static void	ipfw_report(char *txt, int rule, struct ip *ip);
	...
	struct mbuf **mm;
{
	...
	if (stage == IP_FW_GET) {
		...
		*mm = m = m_get(M_WAIT, MT_SOOPTS);
		for (; fcp; fcp = fcp->chain.le_next) {
			memcpy(mtod(m, void *), fcp->rule, sizeof *(fcp->rule));
