...
; 
;-----------------------------------------------------------------------------
; RISC OS Australia Territory
;-----------------------------------------------------------------------------

        GBLS    Help
Help    SETS    "Australian Tr'y"
;                |-------------|   maximum length
...
; Notes:
;  DST = 2am on last Sunday in Oct to 2am on last Sunday in March
; except in 2000 :
;      = 2am on Sun 27 August, to 2am Sun 25 March
;         (STANDARD TIME ACT 1987 No 149,
;          amended STANDARD TIME AMENDMENT ACT 1999 No 13)
;
; Lord Howe Island can be ignored. It's a 5x10km island that has a different
; timezone. There are other islands and areas of different timezones in the
; same area. These are similarly being ignored.
...
NODST2  SETS    "AEST"
        ...
        GBLS    DST1
        GBLS    DST2
DST0    SETS    "AWDT" ; there is no such thing in reality
DST1    SETS    "ACDT"
DST2    SETS    "AEDT"

NODSTOffset0    *       100*60*60*8     ; Eight hours
DSTOffset0      *       100*60*60*9
NODSTOffset1    *       100*60*30*19    ; 9.5 hours
DSTOffset1      *       100*60*30*21
NODSTOffset2    *       100*60*60*10    ; Ten hours
DSTOffset2      *       100*60*60*11
...
IntCurr         SETS    "AUD "          ; international currency symbol
Currency        SETS    "$"             ; currency symbol
; jrf: currency matching common usage, glibc and windows
;      CIA WFB claims A$
...
; The table below should help in this:
                ...
                ; üûùúõöôòóñïîìíëêèéçåãäâàáÜÖÑÉÇÅÄ
                ; øæΩºª∫π∏∑∂µ¥≥≤±∞ØÆ≠¨´™©®ß¶•§£¢°†
                ; ﬂﬁ›€€⁄Ÿÿ◊÷’‘”“—–œŒÕÃÀ …»«∆≈ƒ√¬¡¿
                ; ˇ˛˝¸˚˙˘¯˜ˆıÙÛÚÒÔÓÌÏÎÍÈËÁÊÂ‰„‚·‡
...
UppercaseTable
        ...
        DCD     2_00000100000000000000000000100010 ; Ä
...
LowercaseTable
        ...
        DCD     2_00000000000000000000000000000000 ; @
        DCD     2_00000111111111111111111111111110 ; `
        DCD     2_11001000000000000000000001000100 ; Ä
        DCD     2_00000000000000000000000000000000 ; NBSP
...
AlphaTable
        ...
        DCD     2_00000111111111111111111111111110 ; @
        DCD     2_00000111111111111111111111111110 ; `
        DCD     2_11001100000000000000000001100110 ; Ä
        DCD     2_00000000000000000000000000000000 ; NBSP
        DCD     2_11111111011111111111111111111111 ; ¿
        DCD     2_11111111011111111111111111111111 ; ‡
        MEND
...
PunctuationTable
        ...
        DCD     2_00110011111111111111000000000001 ; Ä
...
SpaceTable
        ...
        DCD     2_00000000000000000000000000000000 ; @
        DCD     2_00000000000000000000000000000000 ; `
        DCD     2_00000000000000000000000000000000 ; Ä
...
AccentedTable
        ...
        DCD     2_00000000000000000000000000000000 ; @
        DCD     2_00000000000000000000000000000000 ; `
        DCD     2_00000000000000000000000001100110 ; Ä
        DCD     2_00000000000000000000000000000000 ; NBSP
...
ForwardFlowTable
        ...
        DCD     2_11111111111111111111111111111111 ; `
        DCD     2_11111111111111111111111111111111 ; Ä
...
; in English. Collation order of – (Icelandic eth), ÿ¯ (O slash), ﬂ (German
; sharp s) and ﬁ˛ (Icelandic thorn) is taken from ISO 6937/2-1983.
...
ToLowerTable
   ...
   DCB    "Ä","Ç","Ç",131,132,"Ü","Ü",135,136,137,138,139,"å","ç","é","è"
   DCB    "ê","ë","í","ì","î","ï","ñ","ó","ò","ô","õ","õ","ú","ù","û","ü"
   DCB    "†","°","¢","£","§","•","¶","ß","®","©","™","´","¨","≠","Æ","Ø"
   DCB    "∞","±","≤","≥","¥","µ","∂","∑","∏","π","∫","ª","º","Ω","æ","ø"
   DCB    "‡","·","‚","„","‰","Â","Ê","Á","Ë","È","Í","Î","Ï","Ì","Ó","Ô"
   DCB    "","Ò","Ú","Û","Ù","ı","ˆ","◊","¯","˘","˙","˚","¸","˝","˛","ﬂ"
   DCB    "‡","·","‚","„","‰","Â","Ê","Á","Ë","È","Í","Î","Ï","Ì","Ó","Ô"
   DCB    "","Ò","Ú","Û","Ù","ı","ˆ","˜","¯","˘","˙","˚","¸","˝","˛","ˇ"
   MEND
...
; OSmith 29-Apr-92 ˇ maps to Y, because there is an upper case equivalent
; of ˇ, it's just that we haven't got a glyph for it. û, ü, and ﬂ on the
...
; even though they are classed as lower case characters.
   ...
   DCB    "`","A","B","C","D","E","F","G","H","I","J","K","L","M","N","O"
   DCB    "P","Q","R","S","T","U","V","W","X","Y","Z","{","|","}","~",127
   DCB    "Ä","Å","Å",131,132,"Ö","Ö",135,136,137,138,139,"å","ç","é","è"
   DCB    "ê","ë","í","ì","î","ï","ñ","ó","ò","ô","ö","ö","ú","ù","û","ü"
   DCB    "†","°","¢","£","§","•","¶","ß","®","©","™","´","¨","≠","Æ","Ø"
   DCB    "∞","±","≤","≥","¥","µ","∂","∑","∏","π","∫","ª","º","Ω","æ","ø"
   DCB    "¿","¡","¬","√","ƒ","≈","∆","«","»","…"," ","À","Ã","Õ","Œ","œ"
   DCB    "–","—","“","”","‘","’","÷","◊","ÿ","Ÿ","⁄","€","‹","›","ﬁ","ﬂ"
...
; but it might as well be right :)
   ...
   DCB    "Ä", 23, 23,131,132, 25, 25,135,136,137,138,139,"å","ç","é","è"
   DCB    "ê","ë","í","ì","î","ï","ñ","ó","ò","ô","ö","õ","ú","ù","û","ü"
   DCB    "†","°","¢",  3,"§","•","¶","ß","®","©","™","´","¨","≠","Æ","Ø"
   DCB    "∞","±","≤","≥","¥","µ","∂","∑","∏","π","∫","ª","º","Ω","æ","ø"
   DCB      1,  1,  1,  1,  1,  1,"∆",  3,  5,  5,  5,  5,  9,  9,  9,  9
   DCB    "–", 14, 15, 15, 15, 15, 15,"◊","ÿ", 21, 21, 21, 21, 25,"ﬁ","ﬂ"
   DCB      1,  1,  1,  1,  1,  1,"Ê",  3,  5,  5,  5,  5,  9,  9,  9,  9
...
ToPlainForCollateTable
   ...
   DCB    "`","a","b","c","d","e","f","g","h","i","j","k","l","m","n","o"
   DCB    "p","q","r","s","t","u","v","w","x","y","z","{","|","}","~",127
   DCB    "Ä","W","w",131,132,"Y","y",135,136,137,138,139,"å","ç","é","è"
   DCB    "ê","ë","í","ì","î","ï","ñ","ó","ò","ô","ö","õ","ú","ù","û","ü"
   DCB    "†","°","¢","£","§","•","¶","ß","®","©","™","´","¨","≠","Æ","Ø"
   DCB    "∞","±","≤","≥","¥","µ","∂","∑","∏","π","∫","ª","º","Ω","æ","ø"
   DCB    "A","A","A","A","A","A","∆","C","E","E","E","E","I","I","I","I"
   DCB    "–","N","O","O","O","O","O","◊","O","U","U","U","U","Y","ﬁ","ﬂ"
   DCB    "a","a","a","a","a","a","Ê","c","e","e","e","e","i","i","i","i"
...
; @Aa¬‚ƒ‰¡·¿‡√„≈Â∆ÊBbCc«ÁDd–Ee ÍÀÎ…È»ËFfûüGgHhIiŒÓœÔÕÌÃÏJjKkLlMm
;  Nn—ÒOo‘Ù÷ˆ”Û“Ú’ıöõÿ¯PpQqRrSsﬂTtﬁ˛Uu€˚‹¸⁄˙Ÿ˘VvWwÅÇXxYyÖÜˇ›˝Zz[\]^_
; `{|}~[DEL]
; ÄÉÑáàâäãåçéèêëíìîïñóòôúù
; °¢£§•¶ß®©™´¨ÆØ∞±π≤≥¥µ∂∑∏∫ªºΩæø
; ◊
; ˜
...
; jrf: To all intents an purposes Australia has the same sorting
;      properties as UK.
