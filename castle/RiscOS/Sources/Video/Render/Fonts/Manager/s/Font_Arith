...
getpixsize Entry ""
 ...
 |
        SMULL   R0,LR,R1,R3
        MOV     R0,R0,LSR #16+9-4
        ORR     R0,R0,LR,LSL #16-9+4
 ]
        MOVS    R4,R0
        RSBMI   R4,R4,#0
        CMP     R4,#&8000
...
transformpt PEntryS TransformPt, "R0-R1,R4-R9"
      ...
      |
        ...
        MOV     R2,R2,LSL R14
        SMULL   R0,R1,R2,R4             ; R0,R1 = oldX * mXX
        MOV     R3,R3,LSL R14
        SMLAL   R0,R1,R3,R6             ; R0,R1 += oldY * mXY
        SMULL   R4,R6,R2,R5             ; R4,R6 = oldX * mYX
        ADD     R2,R8,R0,LSR #16        ; R0,R1 + mX
        SMLAL   R4,R6,R3,R7             ; R4,R6 += oldY * mYY
        ADD     R2,R2,R1,LSL #16        ; R0,R1 + mX
        ADD     R3,R9,R4,LSR #16        ; R4,R6 + mY
        ADD     R3,R3,R6,LSL #16        ; R4,R6 + mY
      ]
        PExitS                          ; doesn't return errors

;.............................................................................

 [ NoARMM ; Inline versions of these routines are used if long multiplies are available
...
scalexcoord Entry "R1"
      ...
      [ NoARMM
        ...
        ADD     R4,R4,R9
      |
        SMULL   R0,R1,R2,R5             ; R0,R1 = oldX * mXX
        SMLAL   R0,R1,R3,R7             ; R0,R1 += oldY * mXY
        ADD     R4,R9,R0,LSR #16        ; + mX
        ADD     R4,R4,R1,LSL #16
      ]
...
scaleycoord Entry "R4"
      ...
      [ NoARMM
        ...
        MOVNE   R0,R3
        BLNE    mul_R0_R1
        ADDNE   R5,R5,R0                ; R5 += oldY * mYY

        ADD     R5,R5,R9
      |
        SMULL   R0,R1,R2,R4             ; R0,R1 = oldX * mYX
        SMLAL   R0,R1,R3,R7             ; R0,R1 += oldY * mYY
        ADD     R5,R9,R0,LSR #16        ; + mY
        ADD     R5,R5,R1,LSL #16
      ]
