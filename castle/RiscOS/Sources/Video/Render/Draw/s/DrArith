...
arith_SSmultD
        Push    "R0-R8"
        SUB     R8,PC,PC                ;R8 = minus 26-bit PSR bits
        LDRB    R0,[R8,R14]!            ;Get first operand
        LDR     R0,[R13,R0,LSL #2]
...
arith_DSmultD
 ...
 [ UseMULL
        MOV     R6,#0
        UMLAL   R6,R7,R0,R4             ;Now add in main low word product
 |
...
arith_DDdivS
        ...
        MOVS    R14,R1,ASR #31          ;Record sign of dividend
...
arith_DDdivS_loop
        ...
        TEQ     R14,#0                  ;Deal with dividend sign by 1's
