 ...
 */
/* Title:   show.c
 * Purpose: show a FontDbox Object
 * Author:  TGR
...
extern _kernel_oserror *show_object (_kernel_swi_regs *r, TaskDescriptor *t)
{
   ...
   if (user_regs->r[0] & 3)
      return make_error (FontDbox_NotPersistent, 0);
...
*/
   ...
   switch (user_regs->r[2]) {
      case 1: /* full show */
         internal->flags     |=  FontDboxInternal_FullInfoShow;
...
*/
   if (internal->flags & FontDboxInternal_GenerateShowEvent) {
      ...
      toolbox_event.hdr.size       =  sizeof (FontDbox_AboutToBeShown_Event);
      toolbox_event.hdr.event_code =  FontDbox_AboutToBeShown;
      toolbox_event.hdr.flags      =  user_regs->r[0];

      about_to_be_shown            = (FontDbox_AboutToBeShown_Event *) &toolbox_event;
