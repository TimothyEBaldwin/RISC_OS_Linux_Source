...
{
    int type, size;
    if (swi (OS_File,  R0, 17,  R1, name,  OUT,  R0, &type,  R4, &size,  END)
        || type == 0)
        return -1;
    return size;
}
                           ...
                           )
{
    ...
    *alpha = -1;
    ...
    err = swi (Font_FindFont,
               R1, font->local,  R2, 16,  R3, 16,  R4, 0,  R5, 0,     /* 1 point for kerning purposes */
               OUT,  R0, &handle,  END);
    if (err) return err;

    if (swi (Font_ReadFontPrefix,  R0, handle,  R1, prefix,  R2, sizeof(prefix),  END))
    {
        lose_font(handle);
        return make_error(EPREFIX);
    }

    if (swi (Font_ReadEncodingFilename,  R0, handle,  R1,  font->encfile,
             R2, sizeof(font->encfile),  END))
    {
        lose_font(handle);
    ...
    if (kern)
    {
        char *kdata;
        int size;
        if (swi (Font_ReadFontMetrics,  R0, handle,  OUT,  R5, &size,  END))
            kdata = NULL;
        else
            kdata = malloc(size);
        if (kdata)
        {
            if (swi (Font_ReadFontMetrics,  R0, handle, R5, kdata,  END))
            {
