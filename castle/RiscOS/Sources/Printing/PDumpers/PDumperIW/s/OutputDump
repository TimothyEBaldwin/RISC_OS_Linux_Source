...
; ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;
; Apple ImageWriter PDumper module
; ---------------------------------
;
...
; --------------------------------------------------------
; Print a 4 digit decimal numger with leading zeros.
; In - r8 = number to output
; Out - all registers preserved
; --------------------------------------------------------

PDumper_PrintDecLead4 EntryS "r0-r2", 8          ; 8 bytes stack for string
        MOV     r0, r8
        MOV     r1, sp
        MOV     r2, #8                  ; 8 bytes of stack
        SWI     XOS_ConvertCardinal2
        PDumper_Error

        SUB     r2, r1, r0              ; Number of digits produced
        RSBS    r2, r2, #4              ; Number of leading zeros required
        BLE     %FT01                   ; Check if no leading zeros
        MOV     r1, #'0'
00
        PDumper_OutputReg r1
        SUBS    r2, r2, #1              ; Decrement until no more zeros
        BNE     %BT00

01
        LDRB    r1, [r0], #1            ; Get first digit (writeback)
02
        PDumper_OutputReg r1
        LDRB    r1, [r0], #1            ; Get next digit (writeback)
        TEQ     r1, #0                  ; Check if we have finshed
        BNE     %BT02                   ; No - go back for the next digit
...
03                                      ; Back for next vert. interlace pass
        ...
        MOV     r4, #255
        MOV     r3, r5
        BL      getiw_length            ; Get the length of a IW line
...
02                                      ; Back for next X interlace pass
        ...
        BL      PDumper_PrintDecLead4   ; OSS Output decimal length
...
20
        ...
        MLA     R8,R1,LR,R6             ; Calculate the total line length
        BL      PDumper_PrintDecLead4   ; OSS Output decimal length
        ...
        PDumper_PrintCountedString R1,R2,R6
20
        BL      sendiw_leading          ; Output margin/excess skip as zeros
...
00                                      ; Back for 8*dht bytes high
        ...
        LDRB    R1,[R0],R5
        TST     R1,R2
        ORRNE   R6,R6,#&80              ;Get byte containing bit and move down a line
        MOV     R6,R6,LSR #1
...
;;<<        LDREQ   r4, [r7, #pd_leftmargin - pd_data]      ; Margin if no skip
        ...
        BLNE    iw_adjust_skip          ; Tramples on mask in r4.
...
00                                      ; Back for 8*dht bytes high
        ...
        TST     R1,R4
        ORRNE   R6,R6,#&80
        MOV     R6,R6,LSR #1            ;Munge in bit from pass correctly
...
;..............................................................................
;
; sendiw_leading .. send leading zeros for the IW dumper
;
...
sendiw_leadingloop
        ...
        B       sendiw_leadingloop      ;Loop back until all sent


;..............................................................................
;
; getiw_length .. obtain length of a scan line, removing any trailing and
; leading zeros.
;
...
getiw_length Entry "R0-R2,R6"
  [ No32bitCode
stack_getiw_r0 * Proc_RegOffset
  |
        SavePSR r2
        Push    r2
stack_getiw_r0 * Proc_RegOffset + 4
  ]
...
getiw_trail_scanline
        SUB     r6, r6, #1      ; Have we finished the line yet?
        CMP     r6, r2
        BLE     getiw_trail_finishedline

        LDRB    LR, [R0, r6]
        TST     LR, r4          ; Any trailing zeros?
        BEQ     getiw_trail_scanline  ; Loop again until either end of line or non-zero byte

getiw_trail_finishedline
        ...
        BEQ     getiw_length_exit


; Now find leading zeros.
        LDR     r0, [sp, #stack_getiw_r0]
        LDRB    r1, [r7, #pd_dump_depth -pd_data]
...
getiw_lead_scanline
        ...
        CMP     r6, r8
        BGE     getiw_lead_finishedline

        LDRB    r2, [r0, r6]
        TST     r2, r4          ; Any leading zeros?
        BEQ     getiw_lead_scanline  ; Loop again until non-zero byte

getiw_lead_finishedline
        MOV     r8, r6          ; Update shortest leading zeros found

        ADD     r0, r0, r3      ; Adjust starting position
        SUBS    r1, r1, #1      ; Have we checked all the scan lines yet?
        BGT     getiw_lead_newline
