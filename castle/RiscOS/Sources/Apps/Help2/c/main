...
static _kernel_oserror* update_help_box(char* string, int length)
{
    ...
    /* Find the size of the 'è ' in the helpbox font */
    _swix(Font_ConverttoOS, _INR(1, 2)|_OUT(1), return_string_length(helpbox_font_handle, "è "), 0,
                                                &bullet_size);
    ...
    for (i=0; i<lines; i++)
    {
        if (buffer_ptr[0] == 'è') wci.icon.bbox.xmin = 0;
