...
static void tx_response(u_char opcode, u_int handle, struct sockaddr_in *from)
{
   ...
   if (mns.mns_flags & MNS_DSINUSE)
      (void)sendtosm(mns.mns_txdsock, m, UNHDRSIZE, 0, 0, (void *) from);
   else
   {
      mns.mns_flags |= MNS_DSINUSE;
      (void)sendtosm(mns.mns_rxdsock, m, UNHDRSIZE, 0, 0, (void *) from);
      mns.mns_flags &= ~MNS_DSINUSE;
   }
 ...
 */
{
   ...
   i_buf[1] = 0;
   i_buf[2] = (char)(Module_Version % 100);
   i_buf[2] = (i_buf[2] % 10) | ((i_buf[2] / 10) << 4); /* BCD */
   i_buf[3] = (char)(Module_Version / 100);
#if Module_Version >= 1000
#error "Calculate top nybble correctly"
#endif
                                    ...
                                    char *data, int len, int type, u_int handle)
{
   ...
   if (mns.mns_flags & MNS_DSINUSE)
      r = sendtosm(mns.mns_txdsock, m, UNHDRSIZE, data, len, (void *) &sin);
   else
   {
      mns.mns_flags |= MNS_DSINUSE;
      r = sendtosm(mns.mns_rxdsock, m, UNHDRSIZE, data, len, (void *) &sin);
      mns.mns_flags &= ~MNS_DSINUSE;
   }
